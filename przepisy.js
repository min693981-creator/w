...
// Helper: oblicza ile dni w tygodniu pojawiło się mleko / ser / jogurt
function obliczNabial(dania) {
    ...
    (dania || []).forEach(function(danie) {
        ...
        const data = danie.data_posilku || danie.data || danie.data_dnia || danie.data_posilku_iso || null;

        if (!data) {
            // Jeśli nie ma daty, spróbuj użyć pola 'data' lub innego rozpoznawalnego klucza;
            // w zbierzObiadyTygodnia często jest zachowana informacja o dniu — jeśli nie, wtedy nie liczmy tego wpisu jako unikalnego dnia
        }

        if (teksty.some(t => t.includes("mleko"))) {
            if (data) dniZMlekiem.add(data);
        }
        if (teksty.some(t => t.includes("ser") || t.includes("twarog") || t.includes("twaróg"))) {
            if (data) dniZSerem.add(data);
        }
        if (teksty.some(t => t.includes("jogurt"))) {
            if (data) dniZJogurtem.add(data);
        }
    });
...
// Helper: czy danie jest oznaczone jako UPF (ultra processed)
function czyDanieJestUPF(danie) {
    if (!danie) return false;
    if (danie.ocena_zgodnosci && danie.ocena_zgodnosci.cechy_dania && danie.ocena_zgodnosci.cechy_dania.ultra_processed) {
        return true;
    }

    // Fallback: przeszukaj nazwy składników
    const teksty = tekstySkladnikow(danie);
    const KLUCZE_UPF = [
        "concentrat", "koncentrat", "syrop", "high fructose", "high corn",
        "flavor", "aromata", "aromat", "emulgator", "barwnik", "konserwant",
        "polisorbat", "sorbian", "sulfite", "parów", "parowk", "parówk", "ketchup", "dżem", "dzem", "musli", "baton"
    ];

    return teksty.some(function(t) {
        return KLUCZE_UPF.some(function(k) { return t.indexOf(k) !== -1; });
    });
}
...
function renderujZgodnoscRozporzadzenie() {
    ...
    if (zebrane.dniZDanymi === 0) {
        kontenerReguly.innerHTML = `<p class="zr-brak-danych">Brak zatwierdzonych posiłków w tym tygodniu &ndash; dodaj i zatwierdź posiłki na stronie „Dziś", aby sprawdzić zgodność z ro[...]
    ...
    // Nowe: policz UPF i nabiał
    const liczUPF = zebrane.dania.filter(function(d) { return czyDanieJestUPF(d); }).length;
    const wynikNabial = obliczNabial(zebrane.dania);
    ...
    const reguly = [
        ...
        // Nowe reguły
        {
            ...
            opis: "Maksymalnie 1 danie z ultra-processed w tygodniu",
            wartosc: liczUPF,
            cel: 1,
            typ: "max"
        },
        {
            tytul: "Mleko i produkty mleczne",
            opis: "Mleko/nabiał powinno pojawić się co najmniej 3 razy tygodniowo",
            wartosc: wynikNabial.mleko,
            cel: 3,
            typ: "min"
        },
        {
